...
#define B_SANDSTORM_SOLAR_BEAM      GEN_LATEST // In Gen5+, Solar Beam can be used in a sandstorm without needing to charge on the first turn, and it won't be affected by the sandstorm on the second turn. Before, it would need to charge and then take damage from the sandstorm.
...
#define B_FLAG_NO_WHITEOUT          FLAG_FINAL_BOSS_LOSS     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
#define B_WAIT_TIME_MULTIPLIER              1    // This determines how long text pauses in battle last. Vanilla is 16. Lower values result in faster battles.
...
// INTRO SLIDE SETTINGS
#define B_INTRO_SLIDE_SPEED     (16) // Controls how fast the sliding into battle intro goes.
#define B_INTRO_TERRAIN_SPEED  (1) // Controls how long the terrain slides until it goes down the screen.
